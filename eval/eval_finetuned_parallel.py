#!/usr/bin/env python
"""
Parallel evaluation script for fine-tuned ERNIE model.
Uses concurrent requests for faster evaluation.

Usage:
    python eval_finetuned_parallel.py --server-url http://localhost:8000/v1 --workers 8
"""

import json
import re
import time
import argparse
from pathlib import Path
from typing import Optional, List, Dict, Tuple
from dataclasses import dataclass, asdict
from concurrent.futures import ThreadPoolExecutor, as_completed
from tqdm import tqdm
from openai import OpenAI

# ============== Configuration ==============
# Use relative paths based on script location
SCRIPT_DIR = Path(__file__).parent.resolve()
DATA_DIR = SCRIPT_DIR.parent / "data" / "splits"
OUTPUT_DIR = SCRIPT_DIR / "results"

MAX_TOKENS = 1500
TEMPERATURE = 0.1
MAX_RETRIES = 3
RETRY_DELAY = 1.0

SEVERITY_LEVELS = ["critical", "major", "minor", "none"]


@dataclass
class EvalResult:
    sample_id: int
    defect_type: str
    ground_truth: str
    predicted: str
    rouge1: float = 0.0
    rouge2: float = 0.0
    rougeL: float = 0.0
    severity_gt: str = ""
    severity_pred: str = ""
    severity_match: bool = False
    yield_gt: Optional[float] = None
    yield_pred: Optional[float] = None
    yield_match: bool = False
    structure_score: float = 0.0
    latency_ms: float = 0.0
    error: Optional[str] = None


def create_client(base_url: str) -> OpenAI:
    return OpenAI(base_url=base_url, api_key="not-needed")


def call_api(client: OpenAI, instruction: str, input_text: str, model: str) -> Tuple[str, float]:
    prompt = f"{instruction}\n\n{input_text}"

    for attempt in range(MAX_RETRIES):
        try:
            start_time = time.time()
            response = client.chat.completions.create(
                model=model,
                messages=[
                    {"role": "system", "content": "You are a senior semiconductor process engineer. Provide detailed root cause analysis."},
                    {"role": "user", "content": prompt}
                ],
                max_tokens=MAX_TOKENS,
                temperature=TEMPERATURE,
            )
            latency = (time.time() - start_time) * 1000
            return response.choices[0].message.content, latency
        except Exception as e:
            if attempt < MAX_RETRIES - 1:
                time.sleep(RETRY_DELAY * (attempt + 1))
            else:
                raise e
    raise Exception("Max retries exceeded")


def extract_severity(text: str) -> str:
    text_lower = text.lower()
    patterns = [r'\*\*severity\*\*[:\s]*(\w+)', r'severity[:\s]*(\w+)']
    for pattern in patterns:
        match = re.search(pattern, text_lower)
        if match:
            sev = match.group(1).strip()
            for level in SEVERITY_LEVELS:
                if level in sev:
                    return level
    for level in ['critical', 'major', 'minor']:
        if level in text_lower:
            return level
    return 'unknown'


def extract_yield_impact(text: str) -> Optional[float]:
    patterns = [
        r'\*\*yield impact\*\*[:\s]*(\d+(?:\.\d+)?)[%\s]',
        r'yield impact[:\s]*(\d+(?:\.\d+)?)[%\s]',
        r'(\d+(?:\.\d+)?)[%\s]*(?:yield|loss|impact)',
    ]
    for pattern in patterns:
        match = re.search(pattern, text.lower())
        if match:
            try:
                return float(match.group(1))
            except:
                continue
    return None


def compute_structure_score(text: str) -> float:
    sections = [r'root cause', r'primary cause', r'corrective action', r'immediate', r'prevention', r'severity', r'yield']
    text_lower = text.lower()
    return sum(1 for s in sections if re.search(s, text_lower)) / len(sections)


# ROUGE scorer singleton
_rouge_scorer = None
def get_rouge_scorer():
    global _rouge_scorer
    if _rouge_scorer is None:
        from rouge_score import rouge_scorer
        _rouge_scorer = rouge_scorer.RougeScorer(['rouge1', 'rouge2', 'rougeL'], use_stemmer=True)
    return _rouge_scorer


def compute_rouge(prediction: str, reference: str) -> Dict[str, float]:
    scorer = get_rouge_scorer()
    scores = scorer.score(reference, prediction)
    return {
        'rouge1': scores['rouge1'].fmeasure,
        'rouge2': scores['rouge2'].fmeasure,
        'rougeL': scores['rougeL'].fmeasure,
    }


def evaluate_single(args) -> EvalResult:
    """Evaluate a single sample - designed for parallel execution."""
    sample_id, sample, server_url, model = args

    client = create_client(server_url)
    instruction = sample['instruction']
    input_text = sample['input']
    ground_truth = sample['output']
    defect_type = sample.get('defect_type', 'unknown')

    try:
        predicted, latency = call_api(client, instruction, input_text, model)
        rouge_scores = compute_rouge(predicted, ground_truth)

        severity_gt = extract_severity(ground_truth)
        severity_pred = extract_severity(predicted)

        yield_gt = extract_yield_impact(ground_truth)
        yield_pred = extract_yield_impact(predicted)
        yield_match = False
        if yield_gt is not None and yield_pred is not None:
            yield_match = abs(yield_gt - yield_pred) <= 5.0

        return EvalResult(
            sample_id=sample_id,
            defect_type=defect_type,
            ground_truth=ground_truth,
            predicted=predicted,
            rouge1=rouge_scores['rouge1'],
            rouge2=rouge_scores['rouge2'],
            rougeL=rouge_scores['rougeL'],
            severity_gt=severity_gt,
            severity_pred=severity_pred,
            severity_match=(severity_gt == severity_pred),
            yield_gt=yield_gt,
            yield_pred=yield_pred,
            yield_match=yield_match,
            structure_score=compute_structure_score(predicted),
            latency_ms=latency,
        )
    except Exception as e:
        return EvalResult(
            sample_id=sample_id,
            defect_type=defect_type,
            ground_truth=ground_truth,
            predicted="",
            error=str(e),
        )


def compute_aggregate_metrics(results: List[EvalResult]) -> dict:
    total = len(results)
    valid = [r for r in results if r.error is None]
    valid_count = len(valid)

    if valid_count == 0:
        return {"total": total, "valid": 0, "error": "No valid results"}

    summary = {
        "total": total,
        "valid": valid_count,
        "errors": total - valid_count,
        "rouge1": round(sum(r.rouge1 for r in valid) / valid_count, 4),
        "rouge2": round(sum(r.rouge2 for r in valid) / valid_count, 4),
        "rougeL": round(sum(r.rougeL for r in valid) / valid_count, 4),
        "severity_accuracy": round(sum(1 for r in valid if r.severity_match) / valid_count, 4),
        "structure_score": round(sum(r.structure_score for r in valid) / valid_count, 4),
        "avg_latency_ms": round(sum(r.latency_ms for r in valid) / valid_count, 1),
    }

    yield_valid = [r for r in valid if r.yield_gt is not None and r.yield_pred is not None]
    summary["yield_accuracy"] = round(sum(1 for r in yield_valid if r.yield_match) / len(yield_valid), 4) if yield_valid else None

    # By defect type
    summary["by_defect_type"] = {}
    for dt in sorted(set(r.defect_type for r in valid)):
        dt_results = [r for r in valid if r.defect_type == dt]
        if dt_results:
            summary["by_defect_type"][dt] = {
                "count": len(dt_results),
                "rouge1": round(sum(r.rouge1 for r in dt_results) / len(dt_results), 4),
                "rougeL": round(sum(r.rougeL for r in dt_results) / len(dt_results), 4),
                "severity_acc": round(sum(1 for r in dt_results if r.severity_match) / len(dt_results), 4),
                "structure": round(sum(r.structure_score for r in dt_results) / len(dt_results), 4),
            }
    return summary


def print_summary(summary: dict, model: str):
    print("\n" + "=" * 60)
    print(f"FINE-TUNED MODEL EVALUATION: {model}")
    print("=" * 60)
    print(f"\nSamples: {summary['valid']}/{summary['total']} valid ({summary['errors']} errors)")

    print(f"\nTEXT SIMILARITY")
    print("-" * 40)
    print(f"  ROUGE-1:  {summary.get('rouge1', 0):.4f}")
    print(f"  ROUGE-2:  {summary.get('rouge2', 0):.4f}")
    print(f"  ROUGE-L:  {summary.get('rougeL', 0):.4f}")

    print(f"\nTASK ACCURACY")
    print("-" * 40)
    print(f"  Severity Match:    {summary.get('severity_accuracy', 0):.1%}")
    yield_acc = summary.get('yield_accuracy')
    print(f"  Yield Impact (±5%): {yield_acc:.1%}" if yield_acc else "  Yield Impact: N/A")
    print(f"  Structure Score:   {summary.get('structure_score', 0):.1%}")

    print(f"\nPERFORMANCE")
    print("-" * 40)
    print(f"  Avg Latency: {summary.get('avg_latency_ms', 0):.0f}ms")

    print(f"\nBY DEFECT TYPE")
    print("-" * 40)
    for dt, m in summary.get("by_defect_type", {}).items():
        print(f"  {dt} (n={m['count']}): ROUGE-L={m['rougeL']:.3f}, Severity={m['severity_acc']:.1%}, Structure={m['structure']:.1%}")


def main(server_url: str, model_name: str, limit: Optional[int], workers: int):
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    print(f"Server: {server_url}")
    print(f"Model: {model_name}")
    print(f"Workers: {workers}")

    # Load data
    with open(DATA_DIR / "test.json") as f:
        test_samples = json.load(f)
    with open(DATA_DIR / "consolidated_all.json") as f:
        consolidated = json.load(f)

    defect_lookup = {s['input']: s.get('defect_type', 'unknown') for s in consolidated}
    for sample in test_samples:
        sample['defect_type'] = defect_lookup.get(sample['input'], 'unknown')

    if limit:
        test_samples = test_samples[:limit]

    print(f"Evaluating {len(test_samples)} samples with {workers} parallel workers...")

    # Prepare tasks
    tasks = [(i, s, server_url, model_name) for i, s in enumerate(test_samples)]
    results = []

    # Run parallel evaluation
    start_time = time.time()
    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = {executor.submit(evaluate_single, task): task[0] for task in tasks}

        with tqdm(total=len(tasks), desc="Evaluating", unit="sample") as pbar:
            for future in as_completed(futures):
                result = future.result()
                results.append(result)

                # Update progress
                valid = [r for r in results if r.error is None]
                if valid:
                    avg_rouge = sum(r.rougeL for r in valid) / len(valid)
                    pbar.set_postfix({"ROUGE-L": f"{avg_rouge:.3f}"})
                pbar.update(1)

    total_time = time.time() - start_time
    print(f"\nTotal time: {total_time:.1f}s ({total_time/len(test_samples):.2f}s/sample)")

    # Sort results by sample_id
    results.sort(key=lambda r: r.sample_id)

    # Save results
    model_safe = model_name.replace("/", "_")
    results_file = OUTPUT_DIR / f"finetuned_{model_safe}_parallel.json"
    with open(results_file, "w") as f:
        json.dump([asdict(r) for r in results], f, indent=2)

    # Compute and save summary
    summary = compute_aggregate_metrics(results)
    summary["model"] = model_name
    summary["workers"] = workers
    summary["total_time_seconds"] = round(total_time, 1)

    summary_file = OUTPUT_DIR / f"summary_finetuned_{model_safe}_parallel.json"
    with open(summary_file, "w") as f:
        json.dump(summary, f, indent=2)

    print_summary(summary, model_name)
    print(f"\nResults: {results_file}")
    print(f"Summary: {summary_file}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--server-url", default="http://localhost:8000/v1")
    parser.add_argument("--model", default="ernie_semi_rca")
    parser.add_argument("--limit", type=int)
    parser.add_argument("--workers", type=int, default=8, help="Number of parallel workers")
    args = parser.parse_args()

    main(args.server_url, args.model, args.limit, args.workers)
