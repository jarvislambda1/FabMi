#!/usr/bin/env python
"""
Evaluation script for fine-tuned ERNIE model on Semiconductor RCA.

This script modifies the original eval_rca.py to work with:
1. Local model served via LLaMA-Factory API
2. Or local model served via vLLM

Usage:
    # Option 1: Start server first, then run eval
    python eval_finetuned.py --server-url http://localhost:8000/v1 --limit 10

    # Option 2: Full evaluation
    python eval_finetuned.py --server-url http://localhost:8000/v1
"""
...
MAX_TOKENS = 1500
TEMPERATURE = 0.1
REQUEST_DELAY = 0.5  # Faster for local inference
MAX_RETRIES = 3
RETRY_DELAY = 2.0
...
def main(
    ...
    results_file = OUTPUT_DIR / f"finetuned_{model_safe}.json"
    ...
    for i, (sample_id, sample) in enumerate(pbar):
        ...
        result = evaluate_sample(client, sample, sample_id, model_name)
    ...
    summary_file = OUTPUT_DIR / f"summary_finetuned_{model_safe}.json"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate fine-tuned model on semiconductor RCA")
    parser.add_argument("--server-url", type=str, default="http://localhost:8000/v1",
                        help="OpenAI-compatible API server URL")
    parser.add_argument("--model", type=str, default="ernie_semi_rca",
